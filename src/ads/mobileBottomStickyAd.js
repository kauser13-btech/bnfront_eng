...
export default function MobileBottomStickyAd({ adData = [], position, catId, w, h, nid }) {
			...
			<div ref={stickyRef} className="bottom-sticky-ad bg-light hide">
				...
				<div className="container position-relative">
					...
					{filteredAds.map((banner, i) => {
						const slotId = `gpt-sticky-mobile-${position}-${banner.id}-${i}`;

						return (
							<div
								key={slotId}
								className="ads mb-2 d-flex justify-content-center ad-sticky-mobile"
								ad-id={banner.id}
							>
								{banner.adtype === 'dfp-code' ? (
									...
									>
										<Image
											...
											alt={banner.ad_img || 'Sticky Mobile Advertisement'}
											width={w}
											height={h}
											quality={100}
