...
const MobileAd = ({ adData, position, catId, w, h, nid }) => {
	...
	return (
		<>
			{isMobile &&
				<div ad-position={position} className={`ad-${w} my-2`}>
					{(adData != '') ?
						<DFPSlotsProvider dfpNetworkId="21675215918">
							{adData.map((banner, i) =>
								(banner.ads_positions_slug == position) ?
									(checkAd(catId, banner) ?
										<div key={i} className={`ads mb-2 d-flex justify-content-center`} ad-id={banner.id}>
