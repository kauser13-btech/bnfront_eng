...
export default function DesktopAd({ adData = [], position, catId, w, h, nid }) {
	...
	const filteredAds = adData.filter(banner =>
		banner.ads_positions_slug === position && checkAd(banner)
	);
	...
	return (
		<>
			{filteredAds.map((banner, i) => {
				const slotId = `gpt-ad-${position}-${banner.id}-${i}`;
